...
/**
 * Provides a block with membership operations the user can perform on a group.
 *
 * @Block(
 *   id = "group_membership_operations",
 *   admin_label = @Translation("Group membership operations"),
 *   context_definitions = {
 *     "group" = @ContextDefinition("entity:group", required = FALSE)
 *   }
 * )
 */
class GroupMembershipOperationsBlock extends GroupOperationsBlock {
  ...
  public function build() {
    ...
    if (($group = $this->getContextValue('group')) && $group->id()) {
      ...
      if ($links) {
        ...
        \Drupal::moduleHandler()->alter('group_members_extra_membership_operations', $links, $group);
